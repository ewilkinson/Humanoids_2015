...
def gau_js(pm, pv, qm, qv):
    ...
    mm = 0.5 * (qm + pm)
    mv = 0.5 * pv + 0.5 * qv + (0.5 * numpy.square(pm) + 0.5 * numpy.square(qm) - numpy.square(0.5 * (pm + qm)))
    imv = numpy.linalg.inv(mv)

    # Determinants of diagonal covariances pv, qv
    dpv = numpy.linalg.det(pv)
    dqv = numpy.linalg.det(qv)
    dmv = numpy.linalg.det(mv)

    # KL(p||m)
    diff = numpy.matrix(mm - pm)
    kl1 = (0.5 *
           (numpy.log(dmv / dpv)  # log |\Sigma_q| / |\Sigma_p|
            + numpy.trace(imv * pv)  # + tr(\Sigma_q^{-1} * \Sigma_p)
            + (diff * imv * diff.transpose())  # + (\mu_q-\mu_p)^T\Sigma_q^{-1}(\mu_q-\mu_p)
            - len(pm)))  # - N
    # KL(q||m)

    diff = numpy.matrix(mm - qm)
    kl2 = (0.5 *
           (numpy.log(dmv / dqv)  # log |\Sigma_p| / |\Sigma_q|
            + numpy.trace(imv * qv)  # + tr(\Sigma_p^{-1} * \Sigma_q)
            + (diff * imv * diff.transpose())  # + (\mu_q-\mu_p)^T\Sigma_p^{-1}(\mu_q-\mu_p)
            - len(pm)))  # - N
    # JS(p,q)
    return 0.5 * (kl1 + kl2)
...
def my_kl(pm, pv, qm, qv):
    inv_qv = numpy.linalg.pinv(qv)
    diff = numpy.matrix(qm - pm)

    det_q = numpy.linalg.det(qv)
    det_p = numpy.linalg.det(pv)

    return (0.5 *
            (numpy.log(det_q / det_p)  # log |\Sigma_q| / |\Sigma_p|
             + numpy.trace(inv_qv * pv)  # + tr(\Sigma_q^{-1} * \Sigma_p)
             + (diff * inv_qv * diff.transpose())  # + (\mu_q-\mu_p)^T\Sigma_q^{-1}(\mu_q-\mu_p)
             - len(pm)))